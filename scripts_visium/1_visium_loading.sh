...
mkdir -p "$PLOT_DIR"
OBJECTS_DIR="$HOME/results/visium/objects"
mkdir -p "$OBJECTS_DIR"
REPORT_DIR="$HOME/results/visium/reports"
mkdir -p "$REPORT_DIR"

# Report file (unique per job)
REPORT_FILE="${REPORT_DIR}/1_visium_loading_report_${SLURM_JOB_ID}.log"
...
#--------------- Run R + capture console + time --------
{
  ...
  Rscript $HOME/scripts_visium/1_visium_loading.R "$PLOT_DIR"
  rstatus=$?
