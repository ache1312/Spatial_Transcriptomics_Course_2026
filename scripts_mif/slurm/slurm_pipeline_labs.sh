...
RUN_PIPELINE_SCRIPT="$PIPELINE_DIR/slurm/run_pipeline_complete_v2.sh"
if [ ! -f "$RUN_PIPELINE_SCRIPT" ]; then
    RUN_PIPELINE_SCRIPT="$PIPELINE_DIR/run_pipeline_complete_v2.sh"
fi

bash "$RUN_PIPELINE_SCRIPT" "$CONDA_ENV"
